...
pub const Image = struct {
    ...
    pub fn loadPpmFile(allocator: std.mem.Allocator, path: []const u8) !Image {
        return loadPpmFromDir(allocator, std.fs.cwd(), path);
    }

    pub fn loadPpmFromDir(allocator: std.mem.Allocator, dir: std.fs.Dir, path: []const u8) !Image {
        ...
        return parseP6(allocator, bytes);
    }
};
...
test "load ppm image" {
    ...
    var image = try Image.loadPpmFromDir(std.testing.allocator, tmp.dir, "tiny.ppm");
    defer image.deinit();

    try std.testing.expectEqual(@as(usize, 2), image.width);
    try std.testing.expectEqual(@as(usize, 1), image.height);
    try std.testing.expectEqual(@as(usize, 6), image.pixels.len);
}
