...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", type=Path, required=True, help="Path to a Ultralytics YOLO .pt checkpoint.")
...
def main() -> None:
    ...
    wrapper = YOLO(args.weights.resolve().as_posix())
    model = wrapper.model.fuse()
    model_name = args.model_name or f"{args.weights.stem}-axionyx"

    graph = {
        "format_version": 1,
        "model_name": model_name,
        "source_weights": str(args.weights.resolve()),
        "input": {"shape": [1, 3, args.input_size, args.input_size], "dtype": "f32"},
        "metadata": {
            "stride": tensor_to_list(model.stride),
            "save": list(model.save),
            "class_count": int(model.yaml["nc"]),
            "class_names": model.names,
            "runtime_family": "ultralytics-yolo-detect",
        },
        "execution_plan": [
            {
                "index": int(layer.i),
