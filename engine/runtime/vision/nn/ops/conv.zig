...
const common = shared_conv.common;
const kernel_3x3 = shared_conv.kernel_3x3;
const kernel_general = shared_conv.general;
const kernel_pointwise = shared_conv.pointwise;
...
fn toSharedOptions(options: Conv2DOptions) common.Conv2DOptions {
    return .{
...
pub fn conv2d(
    ...
    if (!disable_pointwise_fastpath and kernel_h == 1 and kernel_w == 1 and options.stride_h == 1 and options.stride_w == 1 and options.pad_h == 0 and options.pad_w == 0) {
        return kernel_pointwise.conv2dPointwise(input, weights, bias, output, options.groups, options.apply_silu) catch |err| return mapSharedError(err);
    }
    if (!disable_3x3_fastpath and kernel_h == 3 and kernel_w == 3 and options.pad_h == 1 and options.pad_w == 1 and options.groups == 1) {
        return kernel_3x3.conv2d3x3Pad1(input, weights, bias, output, toSharedOptions(options)) catch |err| return mapSharedError(err);
    }
    ...
    if (thread_count > 1) {
        return kernel_general.conv2dGeneralParallel(input, weights, bias, output, toSharedOptions(options), thread_count) catch |err| return mapSharedError(err);
    }

    return kernel_general.conv2dGeneralRange(input, weights, bias, output, toSharedOptions(options), 0, out_channels) catch |err| return mapSharedError(err);
}
...
pub fn conv2dPointwiseConcat(
    ...
    return kernel_pointwise.conv2dPointwiseConcat(inputs, weights, bias, output, apply_silu) catch |err| return mapSharedError(err);
}
